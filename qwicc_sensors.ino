...
#define DISP_LINES 8
#define LINE_LEN 30
#define ALT_FEET 5400 // Altitude adjustment
...
void loop_LPS22() {
  ...
  float adjPressure = pressure.pressure + (ALT_FEET / 30);
  snprintf (buffer[iLineCnt++], LINE_LEN, "%5s : %.2f C", "Temp", temp.temperature);
  snprintf (buffer[iLineCnt++], LINE_LEN, "%5s : %.2f nPa", "Pres", adjPressure);

  sensor[PRES_2].value = adjPressure;
